...
function md5 (arys) { 
    ...
    for(var i = 0; i < newkey.length; i++) {
        //遍历newkey数组
        if(arys[newkey[i]] != "" && arys[newkey[i]] != 0 && arys[newkey[i]] != "0"){ //去除为空为0的值
            newObj[newkey[i]] = encodeBase64Content((arys[newkey[i]]).toString()); 
    ...
    // 打印结果==> {"ac"="acac"&"ba"="baba"&"bb"="bbbb"&"cc"="cccc"}
    var str4 = str3.substring(1,str3.length-1)+app.globalData.appId;
    // 打印结果==> "ac"="acac"&"ba"="baba"&"bb"="bbbb"&"cc"="cccc"

    var str5 = str4.replace(/\s|\xA0/g,"");

    return hexMD5 (str5);
}
// 封装向后台请求的参数
const request_data = function() {
	// let data = {
	// 	requestData:{},
	// 	md5:''
	// };
	// if(arguments[0]) {
	// 	data.requestData = arguments[0];
	// 	data.md5 = md5(arguments[0]);
	// }
	let data = {
		requestData:{},
	};
